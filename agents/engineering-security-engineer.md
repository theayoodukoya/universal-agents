...
server {
    ...
    # SECURITY: Content Security Policy with nonce-based inline style/script support
    # Instead of 'unsafe-inline', use nonce tokens generated per-request for inline styles/scripts
    # The application must set the CSP header dynamically with nonce value in responses
    add_header Content-Security-Policy "default-src 'self'; script-src 'self' 'nonce-DYNAMIC_NONCE'; style-src 'self' 'nonce-DYNAMIC_NONCE'; img-src 'self' data: https:; font-src 'self'; connect-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self';" always;
    # Referrer Policy
