...
def apply_fix_to_cluster(df: pd.DataFrame, column: str, fix: dict) -> pd.DataFrame:
    ...
    # SECURITY: Use ast.literal_eval instead of eval() for sandboxed lambda execution.
    # eval() is dangerous even with validation — use ast.literal_eval for safe literal evaluation.
    import ast
    try:
        transform_fn = ast.literal_eval(fix['transformation'])
        df[column] = df[column].map(transform_fn)
    except (ValueError, SyntaxError) as e:
        # Fallback to quarantine if lambda cannot be safely evaluated
        df['validation_status'] = 'HUMAN_REVIEW'
        df['quarantine_reason'] = f"Lambda evaluation failed: {str(e)}"
        return df
