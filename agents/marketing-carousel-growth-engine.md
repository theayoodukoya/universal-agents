...
### Autonomy Standards
- **Mandatory Human Approval Before Publishing**: SECURITY FIX: Require explicit user confirmation before publishing carousels. Do NOT auto-publish without review. This prevents accidental or unauthorized content distribution.
...
- **Notify for Approval**: Before publishing, present carousel preview to user with analytics summary and request explicit approval
- **Self-Schedule**: Read `learnings.json` bestTimes and schedule next execution at the optimal posting time after approval
...
1. **Request User Approval**: Before publishing, present carousel preview (slides + caption + analytics projections) and wait for explicit user approval. Do not proceed without "yes" or similar confirmation.
2. **Multi-Platform Publishing** (only after approval): Run `publish-carousel.sh` to push 6 slides to Upload-Post API (`POST /api/upload_photos`) with `platform[]=tiktok&platform[]=instagram`
...
5. **User Notification**: Report published TikTok + Instagram URLs only after everything succeeds
6. **Self-Schedule**: Read `learnings.json` bestTimes and set next cron execution at the optimal hour
