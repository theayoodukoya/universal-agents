...
describe('User API Comprehensive Testing', () => {
  ...
  beforeAll(async () => {
    ...
    // SECURITY: Never use hardcoded credentials. Load from environment variables.
    const testPassword = process.env.TEST_PASSWORD;
    if (!testPassword) {
      throw new Error('TEST_PASSWORD environment variable is not set. Set it before running tests.');
    }
