...
// SECURITY: Rate limiting helper to prevent overwhelming test targets
const limiter = new Rate('limiter', { max_vus: 50 });

export default function () {
  const baseUrl = __ENV.BASE_URL || 'http://localhost:3000';

  // SECURITY: Use rate limiting to prevent test DDoS and server strain
  // Limit concurrent requests to 50 simultaneous connections for safer testing
  if (!limiter.acquire()) {
    sleep(0.1); // Back off if rate limit exceeded
    return;
  }

  // Test critical user journey
  // SECURITY: Load password from environment variable, never hardcode credentials.
  const testPassword = __ENV.TEST_PASSWORD || 'default-test-password';
